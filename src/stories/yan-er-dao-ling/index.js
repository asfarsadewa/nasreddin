...
const elements = {
  experience: $('#experience'), stage: $('#canvas-stage'), opening: $('#opening'), openingEyebrow: $('#opening-eyebrow'),
  storyTitle: $('#story-title'), openingDeck: $('#opening-deck'), begin: $('#begin'), beginLabel: $('#begin-label'),
  loadStatus: $('#load-status'), hint: $('.opening__hint'), hud: $('#hud'), taleMark: $('#tale-mark'),
  captionToggle: $('#caption-toggle'), musicToggle: $('#music-toggle'), soundToggle: $('#sound-toggle'), playToggle: $('#play-toggle'),
  chapter: $('#chapter'), chapterLabel: $('#chapter-label'), chapterNumber: $('#chapter-number'), chapterName: $('#chapter-name'),
  captions: $('#captions'), speaker: $('#speaker'), captionLine: $('#caption-line'), timeline: $('#timeline'),
  timelineProgress: $('#timeline-progress'), currentTime: $('#current-time'), totalTime: $('#total-time'),
  ending: $('#ending'), endingEyebrow: $('#ending-eyebrow'), endingTitle: $('#ending-title'), replay: $('#replay'),
  replayLabel: $('#replay-label'), liveStatus: $('#live-status'), languageLaunchers: $$('[data-language-menu]'),
  languageSummaries: $$('[data-language-summary]'), languageScrim: $('#language-scrim'), languagePanel: $('#language-panel'),
  languageClose: $('#language-close'), collectionBacks: $$('[data-collection-back]'), languageEyebrow: $('#language-eyebrow'),
  languageTitle: $('#language-title'), languageIntro: $('#language-intro'), voiceLabel: $('#voice-label'), voiceHelp: $('#voice-help'),
  subtitleLabel: $('#subtitle-label'), subtitleHelp: $('#subtitle-help'), subtitleOff: $('#subtitle-off'),
  languageNames: $$('[data-language-name]'), voiceChoices: $$('[data-voice]'), subtitleChoices: $$('[data-subtitle]'),
};
...
const subtitleCode = (language) => language === 'off' ? 'CC —' : `CC ${language === 'zh' ? '中' : language.toUpperCase()}`;
...
function updateChoiceStates() {
  elements.voiceChoices.forEach((button) => button.setAttribute('aria-checked', String(button.dataset.voice === voiceLanguage)));
  elements.subtitleChoices.forEach((button) => button.setAttribute('aria-checked', String(button.dataset.subtitle === subtitleLanguage)));
}
...
function updatePlaybackControls() {
  ...
  elements.captionToggle.setAttribute('aria-pressed', String(subtitleLanguage !== 'off'));
  elements.captionToggle.setAttribute('aria-label', subtitleLanguage !== 'off' ? currentCopy.hideSubtitles : currentCopy.showSubtitles);
}
...
function localizeInterface() {
  ...
  elements.languageNames.forEach((element) => { element.textContent = currentCopy[element.dataset.languageName]; });
...
function triggerTimedEffects(state) {
...
async function openLanguagePanel(event) {
  ...
  updateLanguageSummary();
  if (audio.started && !audio.paused && !endingShown) { await togglePlayback(false); menuPausedStory = true; }
...
async function closeLanguagePanel(restoreFocus = true) {
  ...
  updateLanguageSummary();
  if (menuPausedStory) { menuPausedStory = false; await togglePlayback(false); }
...
function setSubtitleLanguage(language, announce = true) {
  ...
  if (audio.started) updateInterface(audio.getState());
  if (announce) elements.liveStatus.textContent = language === 'off' ? copy().hideSubtitles : `${copy().subtitles}: ${copy()[{ zh: 'chinese', en: 'english', id: 'indonesian' }[language]]}`;
}
...
elements.voiceChoices.forEach((button) => button.addEventListener('click', async () => {
...
elements.subtitleChoices.forEach((button) => button.addEventListener('click', () => setSubtitleLanguage(button.dataset.subtitle)));
elements.soundToggle.addEventListener('click', () => {
  muted = !muted; audio.setMuted(muted); elements.soundToggle.classList.toggle('is-muted', muted);
  elements.soundToggle.setAttribute('aria-pressed', String(muted)); updatePlaybackControls();
});
elements.musicToggle.addEventListener('click', () => {
  musicMuted = !musicMuted; audio.setMusicMuted(musicMuted); elements.musicToggle.classList.toggle('is-music-muted', musicMuted);
  elements.musicToggle.setAttribute('aria-pressed', String(musicMuted)); elements.liveStatus.textContent = musicMuted ? copy().musicMuted : copy().musicPlaying; updatePlaybackControls();
});
elements.captionToggle.addEventListener('click', () => setSubtitleLanguage(subtitleLanguage === 'off' ? lastSubtitleLanguage : 'off', false));

window.addEventListener('keydown', (event) => {
  if (!elements.languagePanel.hidden && event.key === 'Escape') { event.preventDefault(); closeLanguagePanel(); return; }
